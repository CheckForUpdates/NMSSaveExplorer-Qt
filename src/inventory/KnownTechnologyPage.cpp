...
KnownTechnologyPage::KnownTechnologyPage(QWidget *parent)
    : QWidget(parent)
{
...
bool KnownTechnologyPage::loadFromFile(const QString &filePath, QString *errorMessage)
{
...
bool KnownTechnologyPage::loadFromPrepared(
...
bool KnownTechnologyPage::hasLoadedSave() const
{
    return !currentFilePath_.isEmpty() && !rootDoc_.isNull();
}

bool KnownTechnologyPage::hasUnsavedChanges() const
{
    return hasUnsavedChanges_;
}

const QString &KnownTechnologyPage::currentFilePath() const
{
    return currentFilePath_;
}

bool KnownTechnologyPage::saveChanges(QString *errorMessage)
{
    if (!hasLoadedSave()) {
        if (errorMessage) {
...
void KnownTechnologyPage::clearLoadedSave()
{
...
void KnownTechnologyPage::updateActiveContext()
{
    usingExpeditionContext_ = false;
...
QVariantList KnownTechnologyPage::playerBasePath() const
{
...
void KnownTechnologyPage::applyValueAtPath(const QVariantList &path, const QJsonValue &value)
{
...
bool KnownTechnologyPage::syncRootFromLossless(QString *errorMessage)
{
    return SaveJsonModel::syncRootFromLossless(losslessDoc_, rootDoc_, errorMessage);
}

void KnownTechnologyPage::resetEditor(const QJsonArray &knownTech, const QVariantList &knownPath)
{
    if (editor_) {
        layout()->removeWidget(editor_);
        editor_->deleteLater();
    ...
    editor_ = new KnownTechnologyDialog(knownTech, this);
    layout()->addWidget(editor_);

    connect(editor_, &KnownTechnologyDialog::knownTechChanged, this, [this, knownPath](const QJsonArray &updated) {
        applyValueAtPath(knownPath, updated);
        emit statusMessage(tr("Known technology updated. Pending changes — remember to Save!"));
    });
}
